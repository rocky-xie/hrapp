...
import top.btmdc.hr.config.Constants;
import top.btmdc.hr.domain.User;
import top.btmdc.hr.repository.UserRepository;
import top.btmdc.hr.security.AuthoritiesConstants;
import top.btmdc.hr.service.UserService;
import top.btmdc.hr.service.dto.AdminUserDTO;
import top.btmdc.hr.service.dto.PasswordChangeDTO;
import top.btmdc.hr.web.rest.vm.KeyAndPasswordVM;
import top.btmdc.hr.web.rest.vm.ManagedUserVM;
...
class AccountResourceIT {
    ...
    @Test
    @Transactional
    void testRegisterIsDisabled() throws Exception {
        ManagedUserVM anyUser = new ManagedUserVM();
        anyUser.setLogin("should-not-register");
        anyUser.setPassword("password");
        anyUser.setFirstName("Should");
        anyUser.setLastName("Not");
        anyUser.setEmail("should-not-register@example.com");
        anyUser.setImageUrl("http://placehold.it/50x50");
        anyUser.setLangKey(Constants.DEFAULT_LANGUAGE);
        anyUser.setAuthorities(Set.of(AuthoritiesConstants.USER));

        restAccountMockMvc
            .perform(post("/api/register").contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(anyUser)))
            .andExpect(status().isBadRequest());

        Optional<User> user = userRepository.findOneByLogin("should-not-register");
