...
export default defineComponent({
  ...
  setup() {
    ...
    const retrieveSkillLevels = async () => {
      ...
      try {
        ...
        const res = await skillLevelService().retrieve(paginationQuery);
    ...
    onMounted(async () => {
      await retrieveSkillLevels();
    });

    const removeId: Ref<number> = ref(null);
    const removeEntity = ref<any>(null);
    const prepareRemove = (instance: ISkillLevel) => {
    ...
    const removeSkillLevel = async () => {
      try {
        await skillLevelService().delete(removeId.value);
        const message = `A SkillLevel is deleted with identifier ${removeId.value}`;
